...
- (BOOL)isMobileNumber{
     ...
     */
    NSString *mobileNoRegex = @"^1((3\\d|5[0-9]|8[0-9])\\d|7[0-9]\\d)\\d{7}$";//除4以外的所有个位整数，不能使用[^4,\\d]匹配，这里是否iOS Bug?
...
- (BOOL)isValidateUserName {
    ...
    NSPredicate *name = [NSPredicate predicateWithFormat:@"SELF MATCHES '^[a-zA-Z][a-zA-Z. ]{1,19}|[\u4E00-\u9FA5]{1,10}'"];
    return [name evaluateWithObject:self];
}

- (BOOL)isValidateDecimalOrNumber {
    NSPredicate *number = [NSPredicate predicateWithFormat:@"SELF MATCHES '^[0-9]+([.]{0,1}[0-9]+){0,1}$'"];
    return [number evaluateWithObject:self];
}

- (BOOL)isValidCharatcter {
    NSPredicate *name = [NSPredicate predicateWithFormat:@"SELF MATCHES '^[a-zA-Z. \u4E00-\u9FA5 ]{1,100}'"];
    return [name evaluateWithObject:self];
}
